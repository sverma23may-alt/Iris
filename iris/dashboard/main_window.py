    ...
    def _render_text_pages(self, state: DashboardState) -> None:
        ...
        for execution in executions:
            ...
            workflow_lines.append(
                (
                    f"{execution.get('status', ''):<10} | {execution.get('workflow_name', ''):<24} | "
                    f"step={execution.get('current_step', '--') or '--'} | "
                    f"progress={self._progress_percent(execution, workflows):.0f}% | "
                    f"retries={execution.get('retry_count', 0)}"
                )
            )
        self._set_panel("workflow_timeline", workflow_lines or ["No workflow executions yet."])
        ...
        research_lines = [
            (
                f"{topic.get('title', '')} | score={topic.get('score', 0)} | "
                f"confidence={topic.get('confidence', 0)} | provider={topic.get('source', '')} | "
                f"category={topic.get('category', '')}"
            )
            for topic in topics
            if isinstance(topic, dict)
        ]
        self._set_panel("research", research_lines or ["No trending topics available."])
        ...
        service_lines = [
            f"{service.get('name', ''):<20} | {service.get('status', ''):<10} | healthy={service.get('healthy', False)} | v{service.get('version', '--')}"
            for service in state.services
        ]
        self._set_panel("settings", service_lines)
